...
export default function NavigationLoading() {
  ...
  useEffect(() => {
    ...
    const fadeOutTimer = setTimeout(() => {
      setIsFadingOut(true);
    }, 1000); // Start fading out after 1000ms
    
    const hideTimer = setTimeout(() => {
      setIsLoading(false);
      setIsFadingOut(false);
    }, 1300); // Completely hide after 1300ms
  ...
  return (
    <>
      ...
      {/* Loading Overlay */}
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm ${isFadingOut ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
